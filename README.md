# Alejo GD Official's Github Page

## Welcome to my shitty GitHub page

---

I'm Coder (I think)
- I am currently learning Haxe (I think I know quite a bit about it, but not too much)
- I can program a little bit in Lua

---

I Love Friday Night Funkin' & Geometry Dash

---

I am currently working on a Friday Night Funkin' Engine called [**ALE Engine**](https://github.com/ALE-Engine-Crew), being this currently my *Biggest Project*

---

## Cool People

<p style="display: flex; align-items: center; gap: 8px;">
    Khorix
    <img src="https://github.com/KhorixAwakening.png" height="30" style="margin-right: 10px;">
    Britex
    <img src="https://github.com/IamBritex.png" height="30" style="margin-right: 10px;">
    Dave 27
    <img src="https://github.com/matigol-300.png" height="30" style="margin-right: 10px;">
    Slushi
    <img src="https://github.com/Slushi-Github.png" height="30" style="margin-right: 10px;">
    Ch3erea
    <img src="https://github.com/sparta399.png" height="30" style="margin-right: 10px;">
</p>

---

## Tech Stack

<p style="display: flex; align-items: center; gap: 4px;">
    VS Code
    <img src="images/apps/VSCode.png" height="20" style="margin-right: 10px;">
    Haxe
    <img src="images/apps/Haxe.svg" height="20" style="margin-right: 10px;">
    HaxeFlixel
    <img src="images/apps/HaxeFlixel.svg" height="20" style="margin-right: 10px;">
    Lua
    <img src="images/apps/Lua.png" height="30" style="margin-right: 10px;">
</p>

---

## Stats

<img src="https://github-readme-stats.vercel.app/api?username=AlejoGDOfficial&show_icons=true&theme=nord" />
<br />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=AlejoGDOfficial&layout=compact&show_icons=true&theme=nord" />